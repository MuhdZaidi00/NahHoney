...
function getOrder($db, $orderId) {
    ...
    $sql = 'Select o.customerName, o.orderHoneyName, o.orderDate, o.total FROM orders o  ';
    $sql .= 'Where o.id = :id';
    $stmt = $db->prepare ($sql);
    $id = (int) $orderId;
    $stmt->bindParam(':id', $id, PDO::PARAM_INT);
...
function createOrder($db, $form_data) { 
    //stop at sisni
    $sql = 'Insert into orders ( customerName, orderHoneyName, orderDate, total)'; 
    $sql .= 'values (:customerName, :orderHoneyName, :orderDate, :total)';  
    $stmt = $db->prepare ($sql); 
    $stmt->bindParam(':customerName', $form_data['customerName']);  
    $stmt->bindParam(':orderHoneyName', ($form_data['orderHoneyName']));
...
//update order by id 
function updateOrder($db,$form_data,$orderId) { 

    
    $sql = 'UPDATE orders SET customerName = :customerName , orderHoneyName = :orderHoneyName , orderDate = :orderDate  , total = :total'; 
    $sql .=' WHERE id = :id'; 
    $stmt = $db->prepare ($sql); 
    $id = (int)$orderId;  
    $stmt->bindParam(':id', $id, PDO::PARAM_INT);
